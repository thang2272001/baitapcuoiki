...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='9 Orange - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->to($email);
            $this->email->subject('Hệ thống 9 Orange - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng 9 Orange, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
        ...
        $this->data['title']='9 Orange - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_port']    = '465';
            $config['smtp_timeout'] = '7';
            $config['smtp_user']    = 'sale.Orange.9@gmail.com';
            $config['smtp_pass']    = 'cqfmfmrtudhcmahw';
            $config['charset']    = 'utf-8';
            $config['newline']    = "\r\n";
            $config['wordwrap'] = TRUE;
            ...
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('sale.Orange.9@gmail.com', '9 Orange');
            $this->email->to($list['email']);
            $this->email->subject('Hệ thống 9 Orange - Lấy lại mật khẩu');
        ...
        $this->data['title']='9 Orange - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='9 Orange - Cập nhật mật khẩu mới';
